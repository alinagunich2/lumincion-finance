...
export class MainErnings{
    ...
    processData(){
        ...
        this.data.forEach((itm,index)=>{
            ...
            <span value="${itm.id} ${itm.type==='expense'?'Расход':'Доход'}" class="create">
                <svg value="${itm.id} ${itm.type==='expense'?'Расход':'Доход'}" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path value="${itm.id} ${itm.type==='expense'?'Расход':'Доход'}" d="M12.1465 0.146447C12.3417 -0.0488155 12.6583 -0.0488155 12.8536 0.146447L15.8536 3.14645C16.0488 3.34171 16.0488 3.65829 15.8536 3.85355L5.85357 13.8536C5.80569 13.9014 5.74858 13.9391 5.68571 13.9642L0.68571 15.9642C0.500001 16.0385 0.287892 15.995 0.146461 15.8536C0.00502989 15.7121 -0.0385071 15.5 0.0357762 15.3143L2.03578 10.3143C2.06092 10.2514 2.09858 10.1943 2.14646 10.1464L12.1465 0.146447ZM11.2071 2.5L13.5 4.79289L14.7929 3.5L12.5 1.20711L11.2071 2.5ZM12.7929 5.5L10.5 3.20711L4.00001 9.70711V10H4.50001C4.77616 10 5.00001 10.2239 5.00001 10.5V11H5.50001C5.77616 11 6.00001 11.2239 6.00001 11.5V12H6.29291L12.7929 5.5ZM3.03167 10.6755L2.92614 10.781L1.39754 14.6025L5.21903 13.0739L5.32456 12.9683C5.13496 12.8973 5.00001 12.7144 5.00001 12.5V12H4.50001C4.22387 12 4.00001 11.7761 4.00001 11.5V11H3.50001C3.28561 11 3.10272 10.865 3.03167 10.6755Z" fill="black"/>
                    </svg>                                
            </span>
            </td>
        </tr>`
   ...
   async delete(itm){
    ...
    editPage(){
        ...
        var createElms =  document.getElementsByClassName('create');
        for(var i=0;i<createElms.length;i++){
            createElms[i].addEventListener('click', function(e){
                if(e.target){
                    that.idDelite = e.target.getAttribute('value')
                    Router.id = that.idDelite.split(' ')[0]
                    if(that.idDelite.split(' ')[1]==='Доход'){
                        location.href = '#/edit-ernings-'+that.idDelite.split(' ')[0]
                        // new Creation(that.idDelite.split(' ')[0],'Доход')
                    }else{
                        location.href = '#/edit-comsumption-'+that.idDelite.split(' ')[0]
                        // new Creation(that.idDelite.split(' ')[0],'Расход')
                    }
                }
            })
        } 
